    ...
    protected void internalRegisterService(ServiceInstance<T> service) throws Exception {
        ...
        for (int i = 0; !isDone && (i < MAX_TRIES); ++i) {
            try {
                ...
                isDone = true;
                callBackEvent.onEvent(EventTypes.REGISTRY, service);
                        ...
                        try {
                            ...
                            if (entry != null) {
                                synchronized (entry) {
                                    ...
                                    callBackEvent.onEvent(EventTypes.MODIFY, entry.service);
                                }
                            }
                        } catch (Exception e) {
    ...
    private void internalUnregisterService(final Entry<T> entry) throws Exception {
        if (entry != null) {
            synchronized (entry) {
                ...
                //callback
                callBackEvent.onEvent(EventTypes.UN_REGISTRY, entry.service);
            }
        }
    }
}
